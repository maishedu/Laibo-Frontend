    ...
    <div className="overflow-hidden py-16 bg-black min-h-screen relative h-2/4">
      <div className="px-4  py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
        <div className="  mx-auto flex justify-center lg:max-w-screen-lg">
          <div>
            ...
            <div className="flex flex-col justify-center mt-2">
              <span className=" font-bold default-green  text-lg">
                {"The rich list"}
                <p className="text-neutral-600 font-semibold inline-block ml-4">
                  {userDetails?.rank}
                </p>
              </span>

              <p className="mb-4 text-sm text-white">
                {"(Ranked by library value)"}
              </p>
            </div>
          </div>
        </div>
        ...
        {posts?.map((post, index) => (
          <div
            key={index}
            className="mx-auto  px-5 flex justify-center lg:w-96 "
          >
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <tbody>
                <tr class="flex justify-between ">
                  <td class="px-2 py-2">
                    <Link href={`/profile/${post.username}`}>
                      <span className="text-white font-semibold  text-lg">
                        {post?.rank}
                        <p className="text-neutral-600 font-normal inline-block ml-4">
                  ...
                  <td class="px-2 py-2">
                    <p className="default-green font-semibold ">
                      {post?.total_stock_value.toFixed(2)}
                    </p>
                  </td>
                </tr>
              </tbody>
